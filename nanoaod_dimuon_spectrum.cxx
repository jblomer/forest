...
void nanoaod_dimuon_spectrum() {
  ...
  auto source = RColumnSource::MakeSourceRaw(std::string("data/naod_muon.forest"));
  ROOT::RDataFrame df(std::make_unique<ROOT::RDF::RForestDS>(source.get()));
  //ROOT::RDataFrame df(
  //    "Events",
  //    "data/NanoAOD_DoubleMuon_CMS2011OpenData.root");

  // Select events with more than two muons
  auto df_pre = df.Range(0, 1000);
  auto df_filtered = df_pre.Filter("muons>=2", "More than two muons");
