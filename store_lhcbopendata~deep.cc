...
void Write(RColumnRawSettings settings) {
  using RColumnSink = ROOT::Experimental::RColumnSink;
   ...
   auto kaon_model = std::make_shared<RTreeModel>();
   auto cargo_h_px = kaon_model->Branch<double>("h_px");
   auto cargo_h_py = kaon_model->Branch<double>("h_py");
   auto cargo_h_pz = kaon_model->Branch<double>("h_pz");
   auto cargo_h_prob_k = kaon_model->Branch<double>("h_prob_k");
   auto cargo_h_prob_pi = kaon_model->Branch<double>("h_prob_pi");
   auto cargo_h_charge = kaon_model->Branch<int>("h_charge");
   auto cargo_h_is_muon = kaon_model->Branch<int>("h_is_muon");
   auto cargo_h_ip_chi2 = kaon_model->Branch<double>("h_ip_chi2");
   auto cargo_kaons = event_model->BranchCollection("kaons", kaon_model);
   ...
   for (int i = 0; i < num_events; ++i) {
      ...
      *cargo_h_py       = h1_py;
      *cargo_h_pz       = h1_pz;
      *cargo_h_prob_k   = h1_prob_k;
      *cargo_h_prob_pi  = h1_prob_pi;
      *cargo_h_charge   = h1_charge;
      *cargo_h_is_muon  = h1_is_muon;
      *cargo_h_ip_chi2  = h1_ip_chi2;
      cargo_kaons->Fill();

      *cargo_h_px       = h2_px;
      *cargo_h_py       = h2_py;
      *cargo_h_pz       = h2_pz;
      *cargo_h_prob_k   = h2_prob_k;
      *cargo_h_prob_pi  = h2_prob_pi;
      *cargo_h_charge   = h2_charge;
      *cargo_h_is_muon  = h2_is_muon;
      *cargo_h_ip_chi2  = h2_ip_chi2;
      cargo_kaons->Fill();

      *cargo_h_px       = h3_px;
      *cargo_h_py       = h3_py;
      *cargo_h_pz       = h3_pz;
      *cargo_h_prob_k   = h3_prob_k;
      *cargo_h_prob_pi  = h3_prob_pi;
      *cargo_h_charge   = h3_charge;
      *cargo_h_is_muon  = h3_is_muon;
      *cargo_h_ip_chi2  = h3_ip_chi2;
      cargo_kaons->Fill();
...
int main() {
   RColumnRawSettings settings("data/B2HHH.forest-mem~deep");
   //settings.fCompressionSettings = 104;  // ZLIB, level 4
   //settings.fEpochSize = 64 * 1024 * 1024;
   Write(settings);

   settings.fPath = "data/B2HHH.forest-zlib~deep";
   settings.fCompressionSettings = 104;  // ZLIB, level 4
   Write(settings);
